...
| 10 | Split the five-request and offline results into two separate claims | "verified across five varied requests with no network access" merged two distinct runs: the 5/5 suite ran with network available (the model simply never calls out), while offline execution was confirmed in a separate dead-proxy run. Stating them separately survives the obvious follow-up question. |
